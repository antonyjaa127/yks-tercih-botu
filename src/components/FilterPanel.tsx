...
function DropdownFilter({ label, filterKey, options, selected, onSelect, openDropdown, setOpenDropdown }: DropdownFilterProps) {
  const [search, setSearch] = useState('');
  const showSearch = ['cities', 'universityNames', 'departmentNames'].includes(filterKey);
  ...
  return (
    <div className="mb-2">
      <button type="button" className="w-full flex items-center justify-between px-3 py-2 border border-gray-200 rounded-lg bg-white hover:bg-gray-50 transition-colors" onClick={() => setOpenDropdown(openDropdown === filterKey ? null : filterKey)}>
        <span className="text-xs font-medium text-gray-800 opacity-80">{label}</span>
        {openDropdown === filterKey ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              <div className="border-b border-gray-100 pb-6">
                ...
                {expandedSections.universite && (
                  <div className="space-y-3">
                    <DropdownFilter label="Şehirler" filterKey="cities" options={cities} selected={searchFilters.cities} onSelect={val => handleMultiSelect('cities', val)} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} />
                    <DropdownFilter label="Üniversiteler" filterKey="universityNames" options={universityNames} selected={searchFilters.universityNames} onSelect={val => handleMultiSelect('universityNames', val)} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} />
                    <DropdownFilter label="Bölümler" filterKey="departmentNames" options={departmentNames} selected={searchFilters.departmentNames} onSelect={val => handleMultiSelect('departmentNames', val)} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} />
                  </div>
                )}
              </div>
              {/* Diğer Filtreler */}
              <div className="pb-6">
                ...
                {expandedSections.diger && (
                  <div className="space-y-3">
                    <DropdownFilter label="Burs/Ücret" filterKey="scholarshipLevels" options={[{ label: 'Tam Burslu', value: 'full' }, { label: '%50 Burslu', value: '50' }, { label: '%25 Burslu', value: '25' }, { label: 'Ücretli', value: 'paid' }]} selected={searchFilters.scholarshipLevels} onSelect={val => handleMultiSelect('scholarshipLevels', val)} openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} />
